...
export default function Membership() {
  ...
  const loadData = async () => {
    try {
      ...
      const tRes = await fetch(`${BASE_URL}/api/trainer/all-trainer`);
      const tObj = await tRes.json();
      setTrainers(tObj.data || []);
    } catch (err) {
      console.error("Failed to fetch packages/trainers", err);
  ...
  // post membership
  const postMembership = async (payload) => {
    const res = await fetch(`${BASE_URL}/api/memberships/add-membership`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  const load = async () => {
    ...
    const res = await fetch(`${BASE_URL}/api/customers/${userId}`);
    const obj = await res.json();
    setList(obj.data || {});
  };
